...
## 4. Lab 7 - Gestion des secrets (Bonus : Infisical Vault + Agent)

### Objectif du bonus
- Ne PLUS injecter les secrets directement dans les variables d'environnement du conteneur app.
- Utiliser un **agent** (Infisical Agent) qui recupere les secrets dynamiquement depuis un coffre (Infisical Vault).
- Permettre une **rotation sans restart** (au moins partielle) de l'app.

### Architecture

```
+------------------+     Universal Auth      +------------------+
| Infisical Agent  |  ---------------------> | Infisical Backend|
| (sidecar)        |  <---- secrets -------- | (+ Postgres/Redis)|
+--------+---------+                          +------------------+
         | rend /run/secrets-rendered/app.env (tmpfs volume)
         v
+------------------+
| Flask app        |   secrets_manager.get_secret("API_KEY")
| (SECRETS_FILE=..)|   -> relit le fichier si mtime change (TTL 15s)
+------------------+
```

4 services dans `docker-compose.secrets.yml` :

| Service | Role |
|---------|------|
| `infisical-db` | Postgres 14 (stockage chiffre des secrets) |
| `infisical-redis` | Cache Redis pour Infisical |
| `infisical-backend` | API Infisical (UI sur http://localhost:8080) |
| `infisical-agent` | Sidecar : auth Universal Auth + render template vers volume |
| `app` | Forum Flask, monte le volume en read-only, aucun secret en env |

### Fichiers ajoutes

| Fichier | Role |
|---------|------|
| `docker-compose.secrets.yml` | Stack complete Infisical + app |
| `infisical/agent-config.yaml` | Config de l'agent (auth, sinks, templates) |
| `infisical/app.env.tmpl` | Template Go qui rend le fichier `app.env` |
| `secrets_manager.py` | Loader Python (cache TTL + reload sur mtime) |
| `config.py` (modifie) | `SECRET_KEY = get_secret('SECRET_KEY')` au lieu de `os.environ.get` |

### Comment ca marche

1. **L'agent** s'authentifie aupres du backend via **Universal Auth** (client ID + client secret d'une "machine identity") et recoit un token court.
2. Il evalue le template `app.env.tmpl` et ecrit le resultat dans `/run/secrets-rendered/app.env` (volume `app-secrets`).
3. Polling toutes les 10s : si un secret change dans Infisical, le fichier est reecrit.
4. L'app monte ce volume en **read-only** et lit les secrets via `secrets_manager.get_secret()`.
5. `secrets_manager` cache les valeurs 15s (TTL) et recharge si `mtime` du fichier a change => **rotation partielle sans restart**.

**Limite** : `SECRET_KEY` Flask est capture a l'import de `Config`. Le rotater invalide les sessions existantes => restart necessaire pour cette cle-la. En revanche, tout secret consomme via `get_secret()` au **runtime** (ex. `API_KEY` dans un appel sortant) rotate sans restart.

### Bootstrap (one-shot, manuel)

```bash
# 1. Demarrer seulement le backend Infisical
docker compose -f docker-compose.secrets.yml up -d infisical-db infisical-redis infisical-backend

# 2. Aller sur http://localhost:8080 et creer :
#    - un compte admin
#    - une Organization + un Project "forum-brawlhalla"
#    - un environnement "dev"
#    - 3 secrets : SECRET_KEY, API_KEY, DB_PASSWORD
#    - une Machine Identity avec auth "Universal Auth" et role qui lit le project
#    - copier le Client ID et Client Secret

# 3. Exporter les credentials agent
export INFISICAL_CLIENT_ID="xxxx-xxxx-xxxx"
export INFISICAL_CLIENT_SECRET="yyyy..."

# 4. Demarrer l'agent + l'app
docker compose -f docker-compose.secrets.yml up -d infisical-agent app

# 5. Verifier que le fichier est rendu
docker exec forum-app-bonus cat /run/secrets-rendered/app.env
```

### Tester la rotation sans restart

```bash
# Modifier API_KEY dans l'UI Infisical (http://localhost:8080)
# Attendre 10s (polling agent) + 15s max (TTL secrets_manager)
# => la prochaine lecture via get_secret("API_KEY") renvoie la nouvelle valeur
# => aucun `docker restart` requis
```

### Challenge : remplacer Infisical par OpenBao

Le pattern reste identique, seule la stack Vault change :
- `infisical-backend` -> `openbao` (image `openbao/openbao`)
- `infisical-agent` -> `openbao` en mode agent (`vault agent -config=...`)
- Authentification : **AppRole** (equivalent d'Universal Auth)
- Template : syntaxe [consul-template](https://github.com/hashicorp/consul-template) (`{{ with secret "secret/data/forum" }}...{{ end }}`)
- Le reste (volume partage, `secrets_manager.py`, app) **ne change pas** : l'agent rend le meme `app.env`.

Voir `docker-compose.openbao.yml` (TODO si implemente) pour la variante.
